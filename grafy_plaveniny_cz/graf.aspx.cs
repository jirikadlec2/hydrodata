...
public partial class Default2 : System.Web.UI.Page
{
    ...
    {       
        ...
        {
            Variable varInfo = VariableManager.GetItemById(4);
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {        
            ...
            // validate user input
            PageUrlValidator validator = new PageUrlValidator(Context.Request.QueryString);
            validator.Validate();
            ...
            if ( varInfo.VarEnum == VariableEnum.Precip && interval.Length.TotalDays < 11)
            {
                ...
                if ( CheckHourlyPrecipAvailable(varInfo, stInfo, startDate, endDate) )
                {
                    varInfo = VariableManager.GetItemByEnum(VariableEnum.PrecipHour);
                }
            }

            // assign image url to the chart image
            chartUrl = this.createChartUrl(varInfo.ShortName, stInfo.Id, startDate, endDate);
            ...
            {
                CreateStatistics(varInfo, stInfo, startDate, endDate);
            }
        }
    }

    private bool CheckHourlyPrecipAvailable(Variable var, Station st, DateTime start, DateTime end)
    {
        Variable tmpVar = VariableManager.GetItemByEnum(VariableEnum.PrecipHour);
        ChannelList periods = ChannelManager.GetListByStationAndVariable(st.Id, tmpVar.Id);
    ...
    private string ProcessStation(string stName, Variable var)
    {
       ...
       // StationCollection stColl = new StationCollection(manager);
        ...
        StationList stColl = StationManager.GetListByNameAndVariable(stName, var.Id); 
        ...
        if ( stColl.Count == 0 )
        {
            ...
            stColl = StationManager.GetListByName(stName);
    ...
    private void CreateStatistics(Variable var, Station st, DateTime start, DateTime end)
    {
        Channel ch = ChannelManager.GetItemByStationAndVariable(st.Id, var.Id);
        ITimeSeries ts = TimeSeriesManager.GetTimeSeries(ch, new TimeInterval(start, end));
        
        Statistics stats = StatisticsManager.CreateStatistics(ch, ts);
    ...
    private string CreateLanguageLink(Variable varInfo)
    {
        ...
        {
            case "en-US":
                VariableManager.ChangeCulture(varInfo, new CultureInfo("cs-CZ"));
                LangPathStart = "/cz/" + varInfo.Url;
                break;
            case "cs-CZ":
                VariableManager.ChangeCulture(varInfo, new CultureInfo("en-US"));
        ...
        (PageUrlValidator validator, DateTime start, DateTime end)
    {
        ...
        if ( validator.IsValidStation )
        {
            ...
            {
                noStationVariable = ( cultInfo.Name == "cs-CZ" ? " (chybí mereni)" : " (no data)" );
            ...
            if ( varInfo.VarEnum == VariableEnum.Stage | varInfo.VarEnum == VariableEnum.Discharge )
            {
                if ( stInfo.River != null )
                {
                    ...
                    riverMetaName = ( cultInfo.Name == "cs-CZ" ? "na rece " : "on the river " ) + riverName;
                }
            }
