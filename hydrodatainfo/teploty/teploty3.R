...
site_chmu = 28 #vys brod
site_povodi = 341 #cerna
od = '2012-06-10'
do = '2013-08-01'
datatype <- c("POSIXct")
...
for(day in 1:numdays) {
  ...
  #check diff from povodi
  startdat = chmu$datum[startindex]
  enddat = chmu$datum[endindex]
  tep_ix_povodi <- which(povodi$datum > startdat & povodi$datum <= enddat & !is.na(chmu$teplota))
  tep_ix_chmu <- which(chmu$datum > startdat & chmu$datum <= enddat & !is.na(chmu$teplota))
  
  if (length(tep_ix_povodi) > 12 & length(tep_ix_chmu) > 12) {
    chmu_mean = mean(chmu$teplota[tep_ix_chmu])      
    povodi_mean = mean(povodi$teplota[tep_ix_povodi])
    pov_offset = 5 * round((povodi_mean - chmu_mean) / 5)
    chmu$fixed[startindex:endindex] = chmu$teplota[startindex:endindex] + pov_offset   
  }
}
...
write.table(file_fixed, row.names=FALSE, sep='\t')

plotmin = as.POSIXct('2012-11-12')
