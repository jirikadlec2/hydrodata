    ...
    public class PageUrlValidator
    {
        ...
        private void ValidateVariable()
        {
            ...
            if ( _queryString["var"] != null )
            {
                ...
                Variable var = new Variable(VariableProperty.Url, varUrl);
                if ( var.IsValid )
                {
                    _variable = var;
            ...
            {
                _variable = new Variable(VariableEnum.Stage); //default variable
            }
        }
        ...
        private void ValidateStation()
        {
            ...
            if ( _isValidVariable )
            {
                ...
                Station st = new Station(_manager);
                st.LoadByUrl(stUrl, _variable.Id);
                if ( st.Loaded )
                {
                    _station = st;
                ...
                else
                {
                    ...
                    if ( st.Loaded )
                    {
                        ...
                        _isValidStation = true;
                        _errorMessage =
                            _writer.WriteErrorMessage(st, _variable, MessageType.NoStationVariable);
                    }
                    else
                    {
                        _errorMessage = _writer.WriteErrorMessage(stUrl, MessageType.NoStation);
