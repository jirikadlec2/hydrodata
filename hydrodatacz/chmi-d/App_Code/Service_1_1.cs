   ...
   Namespace = ConstantsNamespace.WS_NAMSPACE,
    Description = WsDescriptions.SvcDevelopementalWarning + WsDescriptions.WsDefaultDescription)]
        ...
        public class Service : WebService, IService
        {
            ...
            {
                //AuthTokenResponseType token = GetAuthTokenObject(userName, password);
                //string xml = WSUtils.ConvertToXml(token, typeof(AuthTokenResponseType));
                //string xml2 = xml.Replace("sitesResponse", "AuthTokenResponse");
                //return string.Empty;
                throw new SoapException("Invalid user name or password", new XmlQualifiedName("authToken"));

            }

            public XmlDocument GetAuthTokenObject(String userName, String password) 
            {
                return new XmlDocument();
                //return ODws.GetAuthToken(userName, password);
            }
