...
namespace WaterOneFlow.odws
{
    ...
    public class WebServiceUtils
    {
        ...
        public static SiteInfoResponseTypeSite[] GetSitesFromDb()
        {
            ...
            using (SqlConnection conn = new SqlConnection(cnn))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    "WHERE lat is not NULL AND operator_id = 1 AND st.st_id IN (SELECT st_id FROM Stationsvariables WHERE var_id in (1, 4, 5, 8, 16) ) ";
        ...
        internal static VariableInfoType GetVariableInfoFromDb(string VariableParameter)
        {
            ...
            //string cnn = GetConnectionString();
            //string variableId = VariableCodeToID(VariableParameter).ToString();
        ...
        private static Decimal convertValue(object val, int varId) 
        {
            ...
            switch (varId)
            {
                ...
                case 5:
                    // discharge
                    return (dVal >= 0) ? Convert.ToDecimal(Math.Round(Math.Pow(2.0f, 0.001 * dVal),3)) : Convert.ToDecimal(-9999.0);
                case 16:
                case 17:
                case 18:
                    // air temperature
